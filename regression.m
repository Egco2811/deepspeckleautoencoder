...
% validationImages = [];
% testImages = [];
% mkdir("trainingGround");
% mkdir("validationGround");
% mkdir("testGround");
...
%% 
%training net
net =  trainNetwork(combinedDs, lgraph_6, options);
save("net", "net");
%% 
%prediction
selectedDs = noisedTestDs;
prediction = predict(net,selectedDs);
for k=1:8  
    idx = randi(numel(selectedDs.Files));
    subplot(2,8,k)
    image(readimage((selectedDs),idx));
    colormap(gray)
